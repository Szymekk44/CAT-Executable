...
namespace CatRunner.Cat.Math
{
            ...
            var precedence = new Dictionary<char, int>
    {
        ...
        { ')', 0 }
            ...
            {
                if (token is double || token is float || token is ulong || token is long || token is uint || token is int || token is ushort || token is short || token is byte || token is bool)
                {
                    output.Add(token);
                }
                ...
                else if (token is char)
                {
                    ...
                    switch (type)
                    {
                        case VarType.Bool:
                            ...
                            break;
                        case VarType.Byte:
                            stack.Push(EvaluateOperation(Convert.ToBoolean(left), Convert.ToBoolean(right), op));
                            break;
                        case VarType.Short:
        ...
        public object EvaluateOperation(bool left, bool right, char op)
        {
            byte left2, right2;
            if (left)
                left2 = 1;
            ...
            switch (op)
            {
                case '+': result = left2 + right2; break;
                case '-': result = left2 - right2; break;
                case '*': result = left2 * right2; break;
                case '/': result = left2 / right2; break;
                case '%': result = left2 % right2; break;
